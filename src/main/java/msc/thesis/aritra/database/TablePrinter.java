...
public class TablePrinter {
    ...
    HashMap<String, Boolean>[] hmRanges ;
    HashMap<String, Boolean>[] hmDomains;
    ...
    HashSet<String> frequentElemURI;
...
* */
    ...
    public void printPropertyRestrictions(String sOutFile, int iInverse) throws SQLException, IOException {
        ...
        // two hashmaps for each property: domain and range
         /*hmRanges = new HashMap[properties.length];
         hmDomains = new HashMap[properties.length];
        ...
        if(hmDomains == null && hmRanges ==null)
            getDomainRange(properties);
    ...
    public void getDomainRange( String []properties)
    {
        hmRanges = new HashMap[  properties.length];
        hmDomains = new HashMap[ properties.length];
    ...
    public void printExistsPropertyMembers(String sOutFile, int iStart, HashSet<String> frequentRoles,
                                           HashSet<String> frequentClasses) throws SQLException, IOException {
    ...
    public boolean printExistsPropertyMembers(int iStart, int iEnd, String sOutFile, HashSet<String> frequentRoles, HashSet<String> frequentClasses) throws SQLException, IOException {
        ...
        ResultSet results = sqlDatabase.query(sQuery1);
        if(frequentElemURI == null)
            getFrequentElem(frequentRoles,frequentClasses);
...
//        System.out.println("TablePrinter: done (" + chunk.size() + ")");
    ...
    public void printAllElConcept(String sOutFile, HashSet<String> frequentRoles,
                                  HashSet<String> frequentClasses) throws SQLException, IOException {
        String properties[] = getProperties();
        if(hmDomains == null && hmRanges ==null)
            getDomainRange(properties);
        ...
        ArrayList<String> chunk = new ArrayList<String>();
        HashMap<String, String> hmPropTops = getExistsPropertyTops(0);
        HashMap<String, String> hmPropBotom= getExistsPropertyTops(1);
        int iDone = 0;
        while (results.next()) {
            //for each individual
            int iIndID = results.getInt("id");
            ...
            // for top and bottom
            for (int i = 0; i < properties.length; i++) {
                if (hmDomains[i].get(sIndURI) != null)
                {
                    String sPropURI = properties[i];
                    String sPropID = hmPropTops.get(sPropURI);
                    sbLine.append(sPropID).append("\t");
                ...
                if(hmRanges[i].get(sIndURI) != null)
                {
                    String sPropURI = properties[i];
                    String sPropID = hmPropBotom.get(sPropURI);
                    sbLine.append(sPropID).append("\t");
                }
            }
            // exist r C
            ResultPairsIterator iter2 =
                    queryEngine.queryPairs(m_sparqlFactory.individualExistsPropertyQuery(sIndURI), frequentElemURI);
            while (iter2.hasNext()) {
                String sPropClass[] = iter2.next();
                String sClass = sPropClass[1];
                String sProp = sPropClass[0];
            ...
            iDone++;
            if (sbLine.length() > 0) {
                System.out.println("TablePrinter.print:  (" + iIndID + ") -> " + sbLine.toString());
                writer.write(sbLine.toString());
                writer.newLine();
            }
        }
        ...
        System.out.println("TablePrinter: done (" + iDone + ")" + new SimpleDateFormat("HH:mm:ss").format(time));
...
// start 19:39 pM
    ...
    public void getFrequentElem(HashSet<String> frequentRoles, HashSet<String> frequentClasses) throws SQLException {
        String frequentProps = getFrequentElemInString(frequentRoles);
        String frequentConcepts = getFrequentElemInString(frequentClasses);
        String sQuery2 = sqlFactory.selectExistsPropertyById(frequentProps);
        ResultSet results2 = sqlDatabase.query(sQuery2);
